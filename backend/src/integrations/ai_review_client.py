...
class AIReviewClient:
    ...
    def _build_suggested_questions(self, *, context: dict[str, Any]) -> list[str]:
        ...
        suggestions = [
            "结合当前图片对象，帮我总结这条记录最需要人工确认的风险点。",
            "根据当前检测结果和缺陷信息，给出人工复核建议。",
        ...
        *,
        ...
        output_contract = (
            ...
            if task_mode == "chat"
                ...
                "回答结构固定为：1. 复核结论；2. 关键证据；3. 风险判断；4. 处置建议；5. 仍需补充的数据或图像。",
        ...
        *,
        ...
        return "\n\n".join(
            [
                ...
                "你必须覆盖：最终判定倾向、关键证据、是否存在误检/漏检风险、是否应继续人工复核、还缺哪些证据。",
                "你必须逐项检查：初检结果与最终结果是否一致、缺陷描述是否支撑当前结论、时间链路是否合理、历史复核是否改变了结论、图像证据是否足以支撑判断、是否需要升级为人工复判。",
                "输出要求：使用 5 个小节，内容要充分，但不要空泛重复；每个小节都应围绕当前记录的真实字段展开。",
                image_status,
                note_block,
                "结构化上下文：",
                context_snapshot,
